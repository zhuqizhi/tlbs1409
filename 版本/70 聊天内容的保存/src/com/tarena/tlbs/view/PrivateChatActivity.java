...
public class PrivateChatActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			ArrayList<Message> list = messageDAO.query(this, friendUser);
	...
	private void addListener() {
		...
		gridView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				try {
					String faceFileName = (String) faceAdapter
							.getItem(position);
	...
	public void showMessage(Context context, Message message) {
		...
		case ChatUtil.TYPE_MAP:
			...
			// 可以用mapView
			ImageView ivMap = (ImageView) view.findViewById(R.id.iv_map);
			...
			ivPlay.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					try {
						...
						String filePath = ChatUtil.getAudioFile()
								.getAbsolutePath();
		...
		case ChatUtil.TYPE_IMAGE:
			// 返回的body原先放的是图的数据，优化后放的是图在sdcard上的文件名
			body = ChatUtil.saveImageToSdcard(body);
		...
		handler.post(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					Log.i("向上移", scrollviewHeight + "," + linearLayoutHeight);
			...
			try {
				...
				if (vector != null) {
					...
					for (Message message : vector) {
						...
						messageDAO.insert(context, message);
	...
	class MyLocationListener implements BDLocationListener {
		...
		public void onReceiveLocation(BDLocation bdLocation) {
			// 坐标是4.9E-324，认为定位失败
			if (bdLocation.getLongitude() != 4.9E-324) {// 取到了正确坐标
	...
	class MyOverLay extends ItemizedOverlay {
		...
		public boolean onTap(GeoPoint geoPoint, MapView mapView) {
			...
			try {
				...
				OverlayItem overlayItem = new OverlayItem(geoPoint, "选中的位置",
						"选中的位置");
				myOverLay.addItem(overlayItem);
				mapView.refresh();
			} catch (Exception e) {
