		...
		tvSubmit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				try {
					...
					// 2,调MapBiz启工作线程从百度服务器上查询地址
					MapBiz mapBiz = new MapBiz();
					mapBiz.queryAddress(latitude, longitude);
					// 3,保存话题。
					// 条件是得到imageAddress和locationAddress
					// imageAddress和locationAddress是在工作线程中得到
					// 肯定，当我们执行mapBiz2.addData，工作线程没有取到数据
					new Thread() {
						@Override
						public void run() {
							try {
								...
								int count = 0;
								while (count < 1000
										&& (imageAddress == null || locationAddress == null)) {
									count++;
									LogUtil.i("CreateTopicReceiver", "count="
											+ count);
								...
								runOnUiThread(new Runnable() {
									...
									public void run() {
										try {
											// 图片上传成不成功，没有得到地址，把话题保存到百度服务器
											MapBiz mapBiz2 = new MapBiz();
											String body = etBody.getText()
													.toString();
											String time = String.valueOf(System
													.currentTimeMillis());
											// 用asyncHttpClient联网,asyncHttpClient源代码new
											// Handler
											mapBiz2.addData(
													TApplication.currentUser,
													body, imageAddress,
													locationAddress, time,
